...
const SDL_Rect& display::main_map_view() const
{
...
{
	const SDL_Rect& rect = main_map_view();
...
const rect_of_hexes display::hexes_under_rect(const SDL_Rect& r) const
{
	...
	SDL_Rect map_rect = main_map_view();
...
{
	return static_cast<int>(main_map_view().x + (loc.x + border_.size) * zoom_ - xpos_);
}

int display::loc_2_screen_y(const map_location& loc) const
{
	return static_cast<int>(main_map_view().y + (loc.y + border_.size) * zoom_ - ypos_);
}
...
{
	return in_theme()? main_map_view(): anim2::rt.rect;
}
...
void display::set_diagnostic(const std::string& msg)
{
	...
	if(msg != "") {
		...
		flabel.set_clip_rect(main_map_rect2());
...
void display::draw_init()
{
	...
	if (redraw_background_) {
		...
		const SDL_Rect clip_rect = main_map_rect2();
	...
	if (invalidateAll_) {
		...
		invalidate_locations_in_rect(main_map_view());
...
void display::announce(const std::string& message, const SDL_Color& color)
{
	...
	flabel.set_position(main_map_rect2().w/2, main_map_rect2().h/3);
...
void display::draw_minimap()
{
	...
	SDL_Rect map_rect = main_map_view();
	SDL_Rect map_out_rect = main_map_rect2();
	double border = border_.size;
	...
	widget->set_blits(gui2::tformula_blit(screen, null_str, null_str, str_cast(area.w), str_cast(area.h)));
}
...
bool display::scroll(int xmove, int ymove)
{
	...
	SDL_Rect dstrect = main_map_view();
	dstrect.x += dx;
	dstrect.y += dy;
	dstrect = intersect_rects(dstrect, main_map_view());
...
void display::set_zoom(int amount)
{
	...
	if (new_zoom != zoom_) {
		SDL_Rect const &area = main_map_view();
...
bool display::tile_fully_on_screen(const map_location& loc)
{
	...
	return !outside_area(main_map_view(), x, y);
}
...
bool display::tile_nearly_on_screen(const map_location& loc) const
{
	int x = loc_2_screen_x(loc);
	int y = loc_2_screen_y(loc);
	const SDL_Rect &area = main_map_view();
...
{
	return invalidate_locations_in_rect(intersect_rects(main_map_view(),rect));
}
